...
Our objective is twofold: (i) to compute the prior term $p$, approximate likelihood term $\softv{\lk}$, and approximate posterior term $f$ (Equation \ref{approxposterior}) from an arbitrary simulator $\pi$, and (ii) to perform replica exchange MCMC to sample from this posterior.

We define a simulator $\pi$ as a program composed arbitrarily of deterministic and stochastic procedures, but where all randomness comes from a set of known random primitives.
...
$\textrm{predexchange}$ (Algorithm \ref{alg:predexchange}) performs replica exchange using $\textrm{softexectute}$ to derive approximate posterior values.
