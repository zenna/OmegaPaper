...
\paragraph{Names}Names passed to random primitives identify independent random draws, and are not the same as variable names in the host programming language.
In Example Program 1, $x$ and $y$ are independent because they use different names $n_x$ and $n_y$.
Care must be taken when a program has loops or is recursive, for example by appending the loop counter to the name.
