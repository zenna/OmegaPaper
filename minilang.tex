...
\begin{center}
  ...
  \end{tabular}
\end{center}
\caption{softexecute on Example Program 1.  Each row shows $\mathbb{D}$,  $p_\mathbb{D}$ and $\log(\softv{\ell}_\mathbb{D})$ just prior to executing line L.  $p_\mathcal{N}$ denotes standard normal pdf.  The final value of $\log(\softv{\ell}_\mathbb{D})$ is $0.9 \softv{<} 0.2 = 700$ at $\alpha = 0.001$.}
\label{table:sofex}
\end{figure}
...
For illustration, if $x = -0.01$ in Example Program 2,
 ...
 $\softv{\ell}$ therefore overapproximates the change required to satisfy $\ell$.
