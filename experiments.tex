...
Figures \ref{fig:density} demonstrate how to truncate a normal distribution through conditioning, while Figure  \ref{gridn} shows samples from a uniform prior  $[-1, 1]^2$ conditioned on a variety of predicates.  While simple, these examples can challenge.


\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.9\linewidth]{gridn5}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.5\linewidth]{truncated}
...
The inverse graphics task aims to sample from a prior distribution over scenes.  A scene is a set of $n \sim \textrm{poisson}(\lambda = 3)$ spheres.
A sphere is parameterized by color, reflectance, emission color, transparency, radius and position, all with a uniform prior.
If $r$ is a render function that maps scenes to image, $i_{obs}$ is an observed image, and $\textrm{nointersect}$ determines if any spheres intersect, the prior is conditioned on the predicate:
...
\begin{figure}
	\centering
	\includegraphics[width=0.9\linewidth]{mylolly.pdf}
	\caption{ Top row: scene renders, bottom row: heatmap of intersections.  The observed scene (left) has no intersections, but without this constraint (middle row) intersections occur.  Conditioning on $\textrm{nointersect}$ eliminates them.}
	\label{fig:invrtmcmc}
\end{figure}
...
To quantitatively compare predicate exchange against approaches existing approaches we constructed an artificial problem that scales to high dimension.
...
Since these methosds are generally not exact, given model parameters $m$, we (i) compute $d = \rho(m< A)$ where $A$ is the conditioning set using the methods developed for predicate exchange, then condition a normal distribution centered on $d$ on 0.
Predicate exchcange  compares favourably in most scenarios
