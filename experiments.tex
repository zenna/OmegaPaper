...
Since these methosds are generally not exact, given model parameters $m$, we (i) compute $d = \rho(m< A)$ where $A$ is the conditioning set using the methods developed for predicate exchange, then condition a normal distribution centered on $d$ on 0 and $\sigma^2 = 1/(2\alpha)$.
