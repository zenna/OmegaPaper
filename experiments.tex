...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.9\linewidth]{gridn5clean}
	\caption{Samples from models using different procedures.  Each model is $x,y \sim \unif{-1,1}$ conditioned from top to bottom on (i) $x \soft{=} y$, (ii) $|x| \soft{>} |y|$, (iii) $x^2 \soft{=} y^2$, and (iv) $\sin(kx)\cos(kx) \soft{>} 0.9999$.
...
\includegraphics[width=0.35\linewidth]{swapmats2}

% \fbox{\includegraphics[width=1\linewidth,trim={0.5cm, .3cm, .5cm, .1cm}, clip]{truncated}}
...
Let $\vect{X} = (X_1, \dots, X_d)$ where $X_i \sim \mathcal{N}(0,1)$ conditioned on an $\epsilon$ thick ring:
\begin{equation}
l(\textbf{x}) = 1 < |\textbf{x}| < 1 + \epsilon
\end{equation}

Table \ref{results} compares the sample average of particle Gibbs (PG), sequential Monte Carlo (SMC), rejection Sampling (RS), and predicate exchange (PE) to the theoretical average of 0.
Since these methods are generally not exact, given model parameters $\vect{x}$, using the methods developed for predicate exchange we (i) compute $d = \rho(\vect{x}, A)$ where $A$ is the conditioning set, then condition a normal distribution: $\mathcal{N}(d, 1/(2\alpha))$.
