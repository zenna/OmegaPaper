...
Since these methods are generally not exact, given model parameters $\textbf{m}$, we (i) compute $d = \rho(m< A)$ where $A$ is the conditioning set using the methods developed for predicate exchange, then condition a normal distribution centered on $d$ and $\sigma^2 = 1/(2\alpha)$.
