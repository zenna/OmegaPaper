...
Predicates can also represent observations that are abstract in the sense that they restrict variables to sets rather than single values.
...
Sampling from models conditioned on predicates presents severe challenges to existing inference procedures, primarly due to the lack of a tractable likelihood function.
...
Consequently, the numerous effective likelihood based sampling \citep{andrieu2003introduction} and variational  \citep{jordan1999introduction, ranganath2014black} approaches to inference are often inapplicable.
...
Predicate exchange addresses shortcomings in probabilistic programming languages, which have vastly expanded the class of probabilistic models that can be expressed, but still heavily restrict the kinds of predicates that can be conditioned on.
