...
\item  \textbf{Replica Exchange} simulates several Markov chains of models conditioned on predicates relaxed to different strengths.  Combined with an accept/reject phase, we are able to draw samples that are asymptotically exact. 
\end{enumerate}
...
The soft Boolean algebra distinguishes predicate relaxation from most other likelihood-free inference procedures, and is necessary to express domain knowledge of composite Boolean structure.
...
Predicates which indicate sets of zero measure -- typically due to equality conditions on continuous variables -- are problematic because the probability of proposing a satisfying state becomes zero.
In these cases predicate exchange must sample at a minimum temperature strictly greater than zero.
In addition, control-flow in a predicate may depends on randomness in the model.
This can cause it to be difficult to evaluate efficiently.
