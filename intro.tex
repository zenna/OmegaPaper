...
Conditioning on predicates such as these allows us to express what should be true without the burden of specifying how.
...
Given a model over time series of glucose levels \citep{levine2017offline,murata2004probabilistic}, this observation can be realized as a predicate that maps the series to 1 if and only if it falls below the threshold.
Neither the occurrence of hypoglycemia nor any of the infinite number of expressible predicates need to exist as model variables apriori.
Probability theory supports conditioning on predicates over glucose values identically to that of concrete values, but sampling from such models remains elusive in practice. 
