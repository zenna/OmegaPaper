...
Observations are by far the most common form of predicate, but in principle a predicate declare any proposition about a domain.
In practice, conditioning on most predicates presents severe challenges to existing inference procedures.

Predicates can express domain knowledge by constraining latent variables.
...
Conditioning a model on this predicate would concentrate probability mass on smaller set of possible latent geometric configurations, ultimately yielding more accurate posterior inferences in the inverse graphics problem.
...
For example, conditioning random variables that are deterministic transformations of other random variables (e.g., the presence of hypoglycemia in the example above, or the sum of variables in a X model) often results in likelihoods that are normalized by intractable integrals.	
...
Continuing the previous example, we may know that a person does \emph{not} have hypoglycemia, or that they have hypoglycemia \emph{or} hyperglycemia, or neither.
...
Predicate exchange is a response to probabilistic programming languages, which have vastly expanded the class of probabilistic models that can be expressed, but have largely not advanced the the kinds of predicates that can be conditioned in practice.
Predicate exchange requires a model transformation;
rather than introduce a new language or mdoeling formalism, we mirror   \cite{wingate2011lightweight} and provide a light-weight implementation that modulates the execution of stochastic simulation based model.
In practice, this means predicate exchange is easily incorporated into most modeling formalisms and frameworks. 

Our approach comes with certain limitations.
Predicates which indicate sets of zero measure -- typically due to equalities on continuous variables -- are problematic because .
...
\begin{enumerate}
	...
	\item Define predicate relaxation criteria (Section \ref{predexchange}) and a logic for relaxed predicates.
	\item Present an algorithm based on replica-exchange Markov Chain Monte Carlo (Section \ref{replicaexchange}) which draws exact samples for positive measure predicates. 
