...
We will assume a probabilistic model $\vect{X} = (X_1, X_2, \dots, X_n)$ is a collection of random variables, and denote a realization (e.g. sample) of the model as $\vect{x} = (x_1, x_2, \dots, x_n)$, where $\vect{x} \in \xp$.
A predicate $\lk(\vect{x})$ is a mapping from realizations of those variables to 0 or 1.
Predicate exchange is composed of two parts:
\begin{enumerate}
...
\item  \textbf{Replica Exchange} is a Markov Chain Monte Carlo procedure that simulates several replicas of $\soft{\lk}$ in parallel. Replica exchange is able to draw exact samples. 
\end{enumerate}
...
\begin{definition}
A function $\soft{\lk} : \xp \to [0, 1]$ parameterized by $\alpha \in [0, \infty)$ is a relaxation of $\lk: \xp \to \{0, 1\}$ if:
\begin{enumerate}[label=(\roman*)]
	\label{def:temp}
	\item For all $\vect{x} \in \xp$, $\lim_{\alpha \to 0}\soft{\lk}(\vect{x}; \alpha) = \lk(\vect{x})$.
	\item For all $\vect{x} \in \xp$, $\lim_{\alpha \to \infty}\soft{\lk}(\vect{x}; \alpha) = 1$.
  \item For all $\alpha < \infty$, $\soft{\lk}(\vect{x}; \alpha) = 1$ iff $\lk(\vect{x}) = 1$.
\end{enumerate}
\end{definition}
...
To construct a soft predicate we define the degree to which a realization $\vect{x}$ satisfies a predicate $\lk$ as the \emph{distance} from $\vect{x}$ to an element in the $\xp$ that satisfies $\ell$.
Concretely, the \emph{weakest} relaxation of $\ell$ is:
...
The predicate $\soft{\lk}_{\inf}(\vect{x})$ is a weakest relaxation of $\ell$ and defined as:
\begin{equation}
...
\paragraph{Composition} We construct $\soft{\lk}$ from $\lk$ compositionally, by substituting primitive predicates (equality, inequalities and logical operators) with soft counterparts.
...
The problem of negation arises because $\soft{\lk}$ is consistent with $Y$ at 1 but not at 0.
...
Let $\cM = (X_1, \dots, X_n)$ be a model, $Y$ be a predicate that conditions $\cM$, and  $\softv{Y}(\omega) = \soft{\lk}(X_1(\omega), ..., X_n(\omega))$ be a relaxation of $Y$.
...
% Both $Y$ and $\soft{\lk}$ are random variables, and hence map from the sample space $\Omega$;
...
% That is, let $\softv{Y}(\omega) = \soft{\lk}(X_1(\omega), ..., X_n(\omega))$, where $X_i \in \cM$.
...
Too high and $\soft{\lk}$ will diverge too greatly from $Y$. Too low and convergence will be slow.
