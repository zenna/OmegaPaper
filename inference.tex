...
A soft predicate $\softv{Y}$ approximates $Y$ in the sense that \todo{IN WHICH WAY?}when viewed as a likelihood function on model parameters, $\softv{Y}$ has a broader support, assigning nonzero weights to parameter values which have zero weight under $Y$.
...
\begin{figure}
  \centering
  \includegraphics[width=0.6\linewidth]{scaling.pdf}
