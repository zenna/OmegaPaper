...
To resolve this, \emph{two-sided} soft primitives yield a pair $(a_0, a_1)$ where $a_0, a_1 \in [0, 1]$.
...
When we condition on a model, however, \rework{still concerned is not informative} we are still concerned on with the true side, i.e. $a_1$ in the pair $(a_0, a_1)$.
...
\begin{definition}
Let $\vect{x}$ be a realization of a model $\vect{X}$, and $p$ denote a prior density. The \emph{approximate posterior} $f$ is the product:
...
For illustration, let $\vect{X} = (\mu, X)$ be a model where $\mu = \beta(3, 4)$ and $X = \mathcal{N}(\mu, 1)$.\todo{Figure was removed}
