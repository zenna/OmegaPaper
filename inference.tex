...
For convenience we define a soft predicate $u$ as a function of its parameters (other random variables in the model) rather than the sample space.
That is, let $\soft{Y}(\omega) = y(X_1, ..., x_n)$ be factored into $u$
Assuming a prior density $p$, the approximate posterior $f$ is the product:
\begin{equation}
f(m) = p(m) \cdot u(m)
\end{equation}
$u$ down weights parameter values which violate $Y$ by the degree to which they violate it. 
This is modulated by temperature.
At maximum temperature $u$ has no effect, and the approximate posterior $f$ is equal to the prior.
...
Since $X_2$ is a deterministic function of $X$ (i) and (ii) we can exclude it for inference.
...
In predicate exchange, $f$ and $g$ correspond to two approximate posteriors (Equation \ref{approxposterior}) at different temperatures.
...
Several good practices are outlined in Following \cite{earl2005parallel}.  In practice, we logarithmically spaced temperature between $0$ and an upper bound, and swap states of chains adjacent in temperature ($\alpha_1$ with $\alpha_2$, $\alpha_2$ with $\alpha_3$, etc) periodically.
