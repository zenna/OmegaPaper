...
These axioms allow us to construct a sampling procedure that (i) samples accurately from local regions, but also (ii) explores large parts of the realization space, and (iii) in some cases can sample from the true posterior without approximation.
...
Instead, to relax $\ell$ we relax the primitive functions it is composed of, which boil down to a small set of numerical and logical primitives.
...
$\alpha$ trades-off between tractability of inference and the fidelity of the approximation.
