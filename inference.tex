...
\paragraph{Graded Satisfiability}$\softv{Y}$ relaxes $Y$ by mapping each realization $m$ of a model to a graded degree.
$\soft{\lk}_{\inf}(m)$ represents the degree to which a model realization $m$ satisfies a predicate. That is, $m = (x_1, \dots, x_n)$ satisfies $Y$ is a function $k_\alpha$ (described below) of its distance to the satisfying set $A = \{(x_1, \dots, x_n) \mid \lk(x_1, \dots, x_n) = 1\}$.
Let $\rho$ be a distance metric, $\soft{\lk}_{\inf}(m)$ is then:
\begin{equation}
