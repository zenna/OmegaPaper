...
\paragraph{Graded Satisfiability}The degree to which a model realization $m = (x_1, \dots, x_n)$ satisfies $Y$ is a function $k_\alpha$ (described below) of the distance $\rho$ of $m$ to the satisfying set $A = \{(x_1, \dots, x_n) \mid \lk(x_1, \dots, x_n) = 1\}$.
Let $\soft{\lk}_{\inf}(m)$ be the smallest such distance:
\begin{equation}
...
\paragraph{Distance} A relaxation kernel $k_\alpha$ bounds potentially unbounded distances to the unit interval, and is paramterized by temperature $\alpha$.
...
\paragraph{Primitives} We construct  (Section \ref{implement}) $\softv{Y}$ from $Y$ compositionally, by substituting primitive predicates (equality, inequalities and logical operators) with soft counterparts.
...
For illustration, let $\cM = (\mu, X)$ be a model where $\mu = \textrm{Beta}(3, 4), X = \mathcal{N}(\mu, 1)$  conditioned on $X = 0.5$.
...
f_\alpha(\mu, x) = \textrm{Beta}_{0,1}(\mu) \cdot \mathcal{N}_{\mu,1}(x) \cdot k_\alpha(\rho(x, 0.5)) 
\end{equation}
