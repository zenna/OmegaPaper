...
Naturally, we are interested in the smallest distance, since it indicates the smallest change we could make to $\vect{x}$.
...
A relaxation kernel is valid if its use in the construction of a soft predicate (Equation \ref{eq;linf}) satisfies the aforementioned relaxation axioms.
...
For example if $x \dsoft{>} 0$ returns $(a_0, a_1)$, then as a function of $x$, $a_1$ is 1 iff $x > 0$ (Figure \ref{negationimg} (a)) and $a_0$ is 1 iff $x < 0$ (Figure \ref{negationimg} (c)).
...
Conditioning on a predicate is however not symmetric on 0 and 1; it restricts the model to the 1 value.
Consequently, we define conditioning on a pair $(a_0, a_1)$ as conditioning on only $a_1$.
