...
\section{Soft Conditioning of Simulation Models}\label{simmodels}
...
Conditioning is defined classically as an operation that restricts a measure space.
However, the probability of an event is equal to the expectation of the function that indicates that event.
This means that we can instead formulate conditioning as an operation between measures.
...
If $Y$ is a predicate, i.e. an indicator function defined on $[0, 1]$ then \ref{eq:softcond} is equivalent to hard conditioning on predicates/events.
\end{definition}
