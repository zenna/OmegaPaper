...
This is difficult, since simulation based models lack an explicit likelihood function, which is a necessary component for most inference procedures.

Simulation based models are random variables in measure theoretic probability:
...
The new model $\cM_{|A}$ can be defined with probability space
\begin{align*}
...
is defined exactly as above, where $A = \{\omega : \lk(X_1(\omega), \dots, X_n(\omega)) = 1\}$,

Sampling from the model $M^*$ generates $X_1, ..., X_n$ where the predicate $\lk	$ is true.
